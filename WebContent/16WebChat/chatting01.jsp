...
<body>
	...
	웹소켓이므로 ws:// 로 시작한다. 마지막경로에는 @serverEndpoint어노테이션으로
	지정했던 요청명을 사용한다.
	*/
		var webSocket = new WebSocket("ws://localhost:8080/K07JSPServlet/ChatServer");
		...
		*/
		...
		webSocket.onmessage = function(event){
			//매개변수를 통해 서버가 보낸 메세지를 출력할 수 없다.
			messageWindow.value += "서버에 수신 =>" + event.data + "\n";
		};
		
		//클라이언트가 "보내기" 버튼을 눌렀을떄 메세지를 서버로 전송하는 메소드
		function sendMessage() {
			//입력창의 DOM을 가져와서...
			var message = document.getElementById("inputMessage");
			//입력된 내용을 출력창에 출력하고...
			messageWindow.value += "서버로 전송 =>" + event.data + "\n";
