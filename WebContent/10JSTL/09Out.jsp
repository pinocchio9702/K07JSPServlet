...
<body>
	...
	<!-- 
	-out태그에 value속성의 값이 null일대는 default속성에
	지정된 값이 출력된다.
	...
	<!-- 
	-파라미터로 전달되는 nowPage가 없는 경우에는 1로 표현하고,
	있는 경우에는 해당 파라미터값을 가져와서 페이지에 적용한다.
	 -->
	<h3>페이지 링크에 응용하기</h3>
	<!-- a태그를 넣어주지 않으면 그냥 주소가 출력됨 -->
	<c:url value="/08Board1/BoardList.jsp?nowPage=" /><c:out 
			value="${param.nowPage }" default="1 " />
	<hr />
	

	<!-- 주의]한줄로 붙여서 작성하게요 줄바꿈하면 공백이 들어갑니다. -->
