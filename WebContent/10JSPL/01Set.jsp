...
<body>
	...
	set태그속성
		...
		scope : 영역을 지정하여 변수를 저장함.
			4가지 영역을 저장할수 있고 기본값은 가장좁은
			page영역
		즉, set태그를 사용하여 변수르 젖아하면 지정된 영역에 저장된다.
	 -->
	<h2>set태그</h2>
	...
	<h3>EL로 출력</h3>
	<!-- EL영역의 속성을 읽올때는 xxScope를 생략하면 가장 좁은
	...
	<!-- 
	set태그의 target, property속성은 자바빈(DTO) 객체의 속성 혹은
	...
	설정한 때는 var속성을 지정할 수 없다.
	 -->
	<h3>target속성과 property속성 사용하기</h3>
	<!-- 1.변수 설정(new를 통해 객체를 만들어 setAttribute()으로 영역에 저장 -->
	<c:set var="defaultMember" value="<%=new MemberDTO() %>" scope="request"  />
	
	<!-- 2. 생성된 변수의 맴버변수값 지정(setter()를 통한 값 설정) -->
	...
	<!-- 
	 ...
	 -->
	<h4>target및 property 속성으로 미 설정후 출력</h4>
	<ul>
