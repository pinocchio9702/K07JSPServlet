...
//웹브라우저에 출력된다. 즉JS의 document.write()와 같은 역할을 한다.
System.out.println("strAge=" + strAge);

int ageAfter10 = -1;
String errorStr = "";
/*
해당 페이지로 폼값을 전송할 떄 "나이" 부분을 공백상태 혹은 숫자가
아닌 문자를 입력후 전송하면 해당 코드에서 에러가 발생된다.
*/
	...
	<!-- 
		form태그의 속성
			...
			enctype : 파일업로드를 위한 속성으로 기술하지 않으면
				파일은 전송할 수 없다. 파일전송을 위해서는 
				multipart/form-date를 지정해야 한다.
	 -->
	<form action="">
		나이를 입력하세요 <input type="text" name="age" /> <input type="submit"
			value="확인" /> <br /> ※나이를 입력하지 않고 전송하면 에러가 발생됩니다.
	</form>
