...
public class PagingUtil {
	public static String paginBS4(int totalRecordCount, int pageSize,
		...
		*/
		...
		if(intTemp != 1) {
			...
			pagingStr += "<li class='page-item><a href='"+pageName + "nowPage=1' class='page-link'><i"
		...
		if(intTemp <= totalPage) {
			...
			//마지막 페이지 바로가기
			pagingStr += "<li class='page-item'><a href='" + pageName + "nowPage="
					+intTemp+"'class='page-link'><i class='fas fa-angle-double-right'></i></a><li>";
		}
	...
	public static String pagingTxt(int totalRecordCount,
			...
			int intTemp = (((nowPage-1) / blockPage) * blockPage) + 1;
			/*
			첫번쨰 페이지 블럭이라면 이전블럭이 존재하지 않으므로 화면상에
			두번째 블럭부터 표시한다.
			*/
			if(intTemp != 1) {
